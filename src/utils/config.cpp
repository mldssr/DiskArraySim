...
static Content *find_content(Content *content, const char *key) {
    ...
    if (strcmp(key, content->key) == 0) return content;
    else return find_content(content->next, key);
}

static Section *find_section(Section *section, const char *name) {
    if (section == NULL) return NULL;

    if (strcmp(name, section->name) == 0) return section;
    else return find_section(section->next, name);
}
            ...
            if (pos == NULL) {
                ...
                *pos = 0;
                ...
                if (find_content(section->contents, name) != NULL) {
                    ...
                    section->contents = new_content(name, value, section->contents);
                    delete value;
                }
                delete name;
            }
...
void Config::read_config(const char *filename) {
    ...
    if (err == NO_ERROR) {
        ...
        if (err == FORMAT_ERROR) {
            fprintf(stderr, "The configuration file %s has unrecognized format!\n", filename);
        } else if (err == DUP_CONTENT) {
            fprintf(stderr, "The configuration file %s has duplicate contents!\n", filename);
        } else if (err == DUP_SECTION) {
            fprintf(stderr, "The configuration file %s has duplicate sections!\n", filename);
        }
    }
}
...
char* Config::get_string(const char *sec, const char *key, const char *deft) {
    ...
    if (section != NULL) {
        ...
        if (content != NULL) value = content->value;
    }
    return value;
}
